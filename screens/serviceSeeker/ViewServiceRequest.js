...
export default function ViewServiceRequest() {
 ...
 const convertDate = (isoString) => {
  ...
  const acceptProposal = async (proposal) => {
      ...
      await database.updateDocument(DATABASE_ID, COLLECTION_IDs.proposals, proposal.proposal_id, {
        status: 'accepted',
      }).then((response) => {
     ...
     await database.updateDocument(DATABASE_ID, COLLECTION_IDs.service_requests, request.$id, {
        status: 'accepted',
        accepted_proposal: proposal.proposal_id,
      }).then((response) => {
...
<View style={{backgroundColor:"#f7f9fc"}}>
      ...
      </View>
     {
      ...
      <TouchableOpacity 
        ...
        onPress={()=>navigation.navigate('ProviderProfile', { providerData: proposal.proposed_user})}  // Navigate to the user profile screen
      >
